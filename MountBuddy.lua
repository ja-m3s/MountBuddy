local MountBuddy = {
  ...
  DEFAULT = { TRAINING_ORDER = { "speed", "stamina", "carrying" } },
  VARIABLE_VERSION = 1,
  DB = {},
  HELP_TEXT = [[
...
function MountBuddy:setTrainingOrder(input)
  if not self.TRAINING_LOOKUP[input] then
    local training_options = "Training Order options are:\n %s"
    local trainTypes = ""
    for key, order in pairs(self.TRAINING_LOOKUP) do
      trainTypes = trainTypes .. key .. ". " .. table.concat(order, ',') .. "\n"
    end
    d(string.format(training_options, trainTypes))
    return
  end
  self.DB.TRAINING_ORDER = self.TRAINING_LOOKUP[input]
  MountBuddy:printTrainingOrder()
end

--print the current mount training
function MountBuddy:printTrainingOrder()
  local strOrder = table.concat(self.DB.TRAINING_ORDER, ',')
...
function MountBuddy:trainMount()
  for _, trainType in ipairs(self.DB.TRAINING_ORDER) do
...
function MountBuddy:onAddonLoad(event, addonName)
  ...
  self.DB = ZO_SavedVars:NewCharacterIdSettings(self.SAVED_VAR_NAME, self.VARIABLE_VERSION, nil, self.DEFAULT)
